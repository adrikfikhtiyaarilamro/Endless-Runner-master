...
try:
    ...
    print(f"✓ BiLSTM-Transformer model loaded from {MODEL_PATH}")
...
# === Logging Setup ===
LOG_FILE = "inference_log_bilstm_transformer.csv"
experiment_id = 0
...
# GUI
root = tk.Tk()
root.title("Voice Command - BiLSTM-Transformer")
...
title = ttk.Label(frame, text=f"Voice Command - BiLSTM-Transformer (16K Pink 40)", font=("Arial", 16, "bold"))
title.grid(column=0, row=0, columnspan=2, pady=10)

model_info = ttk.Label(frame, text=f"Model: BiLSTM-Transformer Hybrid | SR: {SAMPLE_RATE}Hz | MFCC: {N_MFCC}", 
                       font=("Arial", 9))
...
print(f"\n{'='*60}")
print(f"Voice Command Classifier - BiLSTM-TRANSFORMER HYBRID")
print(f"{'='*60}")
print(f"Model: BiLSTM-Transformer (16K Pink 40)")
